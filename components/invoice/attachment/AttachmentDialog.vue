...
<template>
  <Dialog v-model:visible="dialogVisible" modal :header="header" class=" h-fit w-fit"
                  ...
                  <div class="flex align-items-center gap-2">
                    ...
                    <div class="w-full lg:w-auto flex align-items-center">
                      <IconField v-model="filterToSearch.search" icon-position="left" class="w-full">
                        <InputText v-model="filterToSearch.search" type="number" placeholder="Search" 
                          class="w-full" />
    ...
    typeError = false
              ...
              <template #field-file="{ onUpdate, item: data }">
                <FileUpload :max-file-size="1000000" :multiple="false" auto custom-upload @uploader="(event: any) => {
    const file = event.files[0]
    onUpdate('file', file)
  ...
  <Button v-tooltip.top="'View File'" class="w-3rem mx-2 sticky" icon="pi pi-file"
