...
import type { PaginationEvent } from '~/components/table/interfaces/ITableInterfaces'
import type { PageState } from 'primevue/paginator'
import type { Payload } from '~/components/table/interfaces/ITableInterfaces'
import type { Pagination } from '~/components/table/interfaces/ITableInterfaces'
...
function onChangePageOrLimit(event: PageState) {
  props.pagination.page = event.page;
  props.pagination.limit = event.rows;
...
const handleSelectionChange = (event: any) => {
  emits('update:selection', event.value)
}
...
<template>
  ...
  <BlockUI :blocked="options?.loading || parentComponentLoading" class="block-ui-container">
    ...
    <div class="card p-0 mb-0">
      ...
      <DataTable
        ...
        style="border: 0"
        @sort="onSortField"
        @update:selection="(e) => emits('update:selection', e.value)" 
  ...
  <div v-if="'showPagination' in props?.options ? props.options.showPagination : true" class="flex justify-content-center align-items-center mt-2 card py-0">
    ...
    <div v-else class="flex justify-content-between align-items-center w-full">
      ...
      <!-- paginacion -->
      <div v-if="props.pagination" class="flex justify-content-center align-items-center mt-2 card py-0">
        <Paginator
