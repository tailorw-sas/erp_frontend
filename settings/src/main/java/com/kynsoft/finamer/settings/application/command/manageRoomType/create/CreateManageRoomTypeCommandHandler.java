...
public class CreateManageRoomTypeCommandHandler implements ICommandHandler<CreateManageRoomTypeCommand> {
    ...
    public void handle(CreateManageRoomTypeCommand command) {
        ...
        this.producerReplicateManageRoomTypeService.create(new ReplicateManageRoomTypeKafka(command.getId(), command.getCode(), command.getName(), command.getStatus().name(), command.getManageHotel()));
    }
}
