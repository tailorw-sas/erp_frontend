...
import com.kynsoft.finamer.audit.infrastructure.repository.AuditRedisRepository;
...
public class AuditConfigurationServiceImpl implements AuditConfigurationService {
    ...
    private final AuditRedisRepository auditRedisRepository;

    public AuditConfigurationServiceImpl(AuditRedisRepository auditRedisRepository) {
        this.auditRedisRepository = auditRedisRepository;
    }

    @Override
    public PaginatedResponse findAll(Pageable pageable) {
        return getPageResponse(auditRedisRepository.findAll(pageable));
    }

    @Override
    public GetConfigurationByIdResponse findById(UUID id) {
       Optional<AuditConfigurationDto> result= auditRedisRepository.findById(id.toString()).map(AuditConfiguration::toAggregate);
