...
const columns: IColumn[] = [
  ...
  { field: 'firstDigit', header: 'First Digit', type: 'text' },
...
// TABLE OPTIONS -----------------------------------------------------------------------------------------
const options = ref({
  tableName: 'Manage Credit Card Type',
  moduleApi: 'settings',
  uriApi: 'manage-credit-card-type',
  loading: false,
  actionsAsMenu: false,
  messageToDelete: 'Are you sure you want to delete the Credit Card Types {{name}}?'
})
  ...
  if (id) {
    ...
    try {
      ...
      if (response) {
        ...
        item.value.firstDigit = response.firstDigit
      }
