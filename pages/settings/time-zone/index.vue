...
const fields: Array<FieldDefinitionType> = [
  {
    ...
    validation: z.string().trim().min(1, 'The code field is required').min(3, 'Minimum 3 characters').max(5, 'Maximum 5 characters').regex(/^[a-z0-9]+$/i, 'Only alphanumeric characters allowed')
  },
  {
    field: 'name',
    header: 'Name',
  ...
  },
  {
    ...
    dataType: 'text',
    class: 'field col-12',
    // validation: z.number().nullable().refine(value => value !== null, { message: 'The elapse field is required' }),
  },
  {
    field: 'description',
    header: 'Description',
...
const columns: IColumn[] = [
  ...
  { field: 'elapse', header: 'Elapse', type: 'text' },
  ...
  if (id) {
    ...
    try {
      ...
      if (response) {
        ...
        item.value.elapse = response.elapse
      }
...
onMounted(() => {
  ...
  if (useRuntimeConfig().public.loadTableData) {
    getList()
    document.title = 'Manage Time Zone'
  }
})
// -------------------------------------------------------------------------------------------------------
</script>
    ...
    <div class="col-12 order-1 md:order-0 md:col-6 xl:col-3">
      <div>
        ...
        <div class="card p-4">
          ...
          >
            <template #field-elapse="{ item: data, onUpdate }">
              <InputNumber
                v-if="!loadingSaveAll" v-model="data.elapse" :max-fraction-digits="5"
                @update:model-value="($event) => {
                  onUpdate('elapse', $event)
                }"
              />
