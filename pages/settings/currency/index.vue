...
// TABLE OPTIONS -----------------------------------------------------------------------------------------
const options = ref({
  tableName: 'Manage Currency',
  moduleApi: 'settings',
  uriApi: 'manage-currency',
  loading: false,
  actionsAsMenu: false,
  messageToDelete: 'Are you sure you want to delete the currency: {{name}}?'
})
...
async function getItemById(id: string) {
  if (id) {
    ...
    try {
      ...
      if (error) {
        toast.add({ severity: 'error', summary: 'Error', detail: 'Currency methods could not be loaded', life: 3000 })
      }
    }
    finally {
...
onMounted(() => {
  ...
  if (useRuntimeConfig().public.loadTableData) {
    getList()
    document.title = 'Manage Currency'
  }
})
// -------------------------------------------------------------------------------------------------------
</script>
