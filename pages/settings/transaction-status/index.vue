...
const options = ref({
  ...
  loading: false,
  actionsAsMenu: false,
  messageToDelete: 'Are you sure you want to delete the transaction status: {{name}}?'
})
...
async function getItemById(id: string) {
  ...
  try {
    ...
    console.error(error)
    toast.add({ severity: 'error', summary: 'Error', detail: 'Transaction status methods could not be loaded', life: 3000 })
  }
  finally {
...
onMounted(() => {
  ...
  if (useRuntimeConfig().public.loadTableData) {
    getList()
    document.title = 'Manage Transaction Status'
  }
})
// -------------------------------------------------------------------------------------------------------
</script>
