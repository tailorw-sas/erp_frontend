...
const fields: Array<FieldDefinitionType> = [
  ...
  },
  {
    ...
    validation: z.string().trim().min(1, 'The name field is required')
  },
  {
    field: 'navigate',
    header: 'Navigate',
  ...
  {
    field: 'enabledPayment',
    header: 'Enabled Payment',
    dataType: 'check',
...
const navItems = computed(() => {
  return idItem.value ? [...navigateListItems.value.filter((item: any) => item.id !== idItem.value)] : navigateListItems.value
})
// -------------------------------------------------------------------------------------------------------

// TABLE OPTIONS -----------------------------------------------------------------------------------------
const options = ref({
  tableName: 'Manage Collection Status',
  moduleApi: 'settings',
  uriApi: 'manage-collection-status',
  loading: false,
  actionsAsMenu: false,
  ...
  if (id) {
    ...
    try {
      ...
      if (response) {
        ...
        item.value.enabledPayment = response.enabledPayment
        item.value.isVisible = response.isVisible
  ...
  <div class="grid">
    <div class="col-12 md:order-1 md:col-6 xl:col-9">
      ...
      <DynamicTable
        ...
        @on-confirm-create="clearForm" @update:clicked-item="getItemById($event)"
        @open-edit-dialog="getItemById($event)" @on-change-pagination="payloadOnChangePage = $event"
