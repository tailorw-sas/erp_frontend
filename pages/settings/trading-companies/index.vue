...
const confApi = reactive({
  moduleApi: 'settings',
  uriApi: 'manage-trading-companies',
})
...
const fields: Array<FieldDefinitionType> = [
  ...
  },
  {
    ...
    dataType: 'text',
    class: 'field col-12 required',
    validation: z.string().trim().min(1, 'The CIF field is required')
  },
  {
    field: 'company',
    header: 'Company',
    dataType: 'text',
    class: 'field col-12 required',
    validation: z.string().trim().min(1, 'The Company field is required')
  },
  {
    field: 'address',
    header: 'Address',
    dataType: 'text',
    class: 'field col-12 required',
    validation: z.string().trim().min(1, 'The Address field is required')
  },
  {
    field: 'country',
    header: 'Country',
  ...
  },
  {
    ...
    dataType: 'select',
    class: 'field col-12 required',
    validation: validateEntityStatus('cityState'),
  },
  {
    field: 'city',
    header: 'City',
    dataType: 'text',
    class: 'field col-12 required',
    validation: z.string().trim().min(1, 'The City field is required')
  },
  {
    field: 'zipCode',
    header: 'Zip Code',
  ...
  {
    field: 'isApplyInvoice',
    header: 'Apply Invoice',
    dataType: 'check',
...
const columns: IColumn[] = [
  ...
  { field: 'company', header: 'Name', type: 'text' },
...
// TABLE OPTIONS -----------------------------------------------------------------------------------------
const options = ref({
  tableName: 'Manage Trading Companies',
  moduleApi: 'settings',
  uriApi: 'manage-trading-companies',
  loading: false,
  actionsAsMenu: false,
  ...
  if (id) {
    ...
    try {
      ...
      if (response) {
        ...
        item.value.zipCode = String(response.zipCode)
...
onMounted(() => {
  ...
  if (useRuntimeConfig().public.loadTableData) {
    getList()
    document.title = 'Manage Trading Companies'
  }
})
// -------------------------------------------------------------------------------------------------------
</script>
          ...
          >
            ...
            <template #field-cityState="{ item: data, onUpdate }">
              <DebouncedAutoCompleteComponent
                ...
                @load="($event) => getCityStatesList(data.country.id, $event)"
              />
              <Skeleton v-else height="2rem" class="mb-2" />
            </template>
            <template #form-footer="props">
              <div class="flex justify-content-end">
                <IfCan :perms="idItem ? ['TRADING-COMPANY:EDIT'] : ['TRADING-COMPANY:CREATE']">
                  <Button v-tooltip.top="'Save'" class="w-3rem mx-2" icon="pi pi-save" :loading="loadingSaveAll" @click="props.item.submitForm($event)" />
                </IfCan>
                <IfCan :perms="['TRADING-COMPANY:DELETE']">
