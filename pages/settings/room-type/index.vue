...
const fields: Array<FieldDefinitionType> = [
  {
    ...
    validation: z.string().trim().min(1, 'The code field is required').min(1, 'Minimum 1 characters').max(10, 'Maximum 10 characters').regex(/^[a-z]+$/i, 'Only text characters allowed')
  },
  {
    field: 'name',
    header: 'Name',
...
async function getItemById(id: string) {
  if (id) {
    ...
    try {
      ...
      if (error) {
        toast.add({ severity: 'error', summary: 'Error', detail: 'Hotel could not be loaded', life: 3000 })
      }
    }
    finally {
...
function onSortField(event: any) {
  if (event) {
    payload.value.sortBy = event.sortField === 'manageHotel' ? 'manageHotel.name' : event.sortField
...
onMounted(() => {
  ...
  if (useRuntimeConfig().public.loadTableData) {
    getList()
    document.title = 'Manage Room Type'
  }
})
// -------------------------------------------------------------------------------------------------------
</script>
          ...
          <AccordionTab>
            ...
            <div class="flex gap-4 flex-column lg:flex-row">
              ...
              <div class="flex align-items-center">
                <Button
                  v-tooltip.top="'Search'" :disabled="disabledSearch" class="w-3rem mx-2" icon="pi pi-search"
