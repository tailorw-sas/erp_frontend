...
const fields: Array<FieldDefinitionType> = [
  {
    ...
    headerClass: 'mb-1',
    validation: z.string().trim().min(1, 'The code field is required').min(2, 'Minimum 2 characters').max(5, 'Maximum 5 characters').regex(/^[a-z]+$/i, 'Only text characters allowed')
  },
  {
    field: 'name',
    header: 'Name',
...
const confLanguageApi = reactive({
  moduleApi: 'settings',
  uriApi: 'manage-language',
})
...
// TABLE OPTIONS -----------------------------------------------------------------------------------------
const options = ref({
  tableName: 'Manage Country',
  moduleApi: 'settings',
  uriApi: 'manage-country',
  loading: false,
  actionsAsMenu: false,
  messageToDelete: 'Are you sure you want to delete the country: {{name}}?'
})
...
async function getItemById(id: string) {
  if (id) {
    ...
    try {
      ...
      if (error) {
        toast.add({ severity: 'error', summary: 'Error', detail: 'Country methods could not be loaded', life: 3000 })
      }
    }
    finally {
...
async function getListLanguage(query: string) {
  try {
    ...
    const response = await GenericService.search(confLanguageApi.moduleApi, confLanguageApi.uriApi, payload)
    ...
    <div class="col-12 order-1 md:order-0 md:col-6 xl:col-3">
      <div>
        ...
        <div class="card">
          ...
          >
            <template #field-managerLanguage="{ item: data, onUpdate }">
              <DebouncedAutoCompleteComponent
                v-if="!loadingSaveAll"
                id="autocomplete"
                field="name"
                item-value="id"
                :model="data.managerLanguage"
                :suggestions="languageList"
                @change="($event) => {
                  onUpdate('managerLanguage', $event)
                }"
                @load="($event) => getListLanguage($event)"
              />
              <Skeleton v-else height="2rem" class="mb-2" />
            </template>
            <template #form-footer="props">
              <div class="flex justify-content-end">
                <IfCan :perms="idItem ? ['COUNTRY:EDIT'] : ['COUNTRY:CREATE']">
                  <Button v-tooltip.top="'Save'" class="w-3rem mx-2" icon="pi pi-save" :loading="loadingSaveAll" @click="props.item.submitForm($event)" />
                </IfCan>
                <IfCan :perms="['COUNTRY:DELETE']">
