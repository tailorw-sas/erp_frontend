...
const fields: Array<FieldDefinitionType> = [
  ...
  },
  {
    ...
    header: 'Min Number Of Characters',
    dataType: 'number',
    class: 'field col-12',
    disabled: true,
    validation: z.number().nullable().refine(value => value !== null, { message: 'The min number of characters field is required' }),
  },
  {
    field: 'defaultRemark',
...
// TABLE OPTIONS -----------------------------------------------------------------------------------------
const options = ref({
  tableName: 'Manage Invoice Transaction Type',
  moduleApi: 'settings',
  uriApi: 'manage-invoice-transaction-type',
  loading: false,
  actionsAsMenu: false,
...
onMounted(() => {
  ...
  if (useRuntimeConfig().public.loadTableData) {
    getList()
    document.title = 'Manage Invoice Transaction Type'
  }
})
// -------------------------------------------------------------------------------------------------------
</script>
    ...
    <div class="col-12 order-1 md:order-0 md:col-6 xl:col-3">
      <div>
        ...
        <div class="card p-4">
          ...
          >
            <template #field-isRemarkRequired="{ item: data, onUpdate }">
              <Checkbox
                ref="refRemarkRequired"
                v-model="data.isRemarkRequired"
                :binary="true"
                @update:model-value="($event) => {
                  onUpdate('isRemarkRequired', $event)
                  updateFieldProperty(fields, 'minNumberOfCharacters', 'disabled', !$event)
