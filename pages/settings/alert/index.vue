...
// TABLE OPTIONS -----------------------------------------------------------------------------------------
const options = ref({
  tableName: 'Manage Alert',
  moduleApi: 'settings',
  uriApi: 'manage-alert',
  loading: false,
  actionsAsMenu: false,
  messageToDelete: 'Are you sure you want to delete the alert: {{name}}?'
})
  ...
  if (id) {
    ...
    try {
      ...
      if (response) {
        ...
        item.value.popup = response.popup
      }
      ...
      if (error) {
        toast.add({ severity: 'error', summary: 'Error', detail: 'Alert methods could not be loaded', life: 3000 })
      }
    }
    finally {
