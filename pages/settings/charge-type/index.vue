...
// TABLE OPTIONS -----------------------------------------------------------------------------------------
const options = ref({
  tableName: 'Manage Charge Type',
  moduleApi: 'settings',
  uriApi: 'manage-charge-type',
  loading: false,
  actionsAsMenu: false,
  messageToDelete: 'Are you sure you want to delete the charge type: {{name}}?'
})
...
async function getItemById(id: string) {
  if (id) {
    ...
    try {
      ...
      if (error) {
        toast.add({ severity: 'error', summary: 'Error', detail: 'Charge type methods could not be loaded', life: 3000 })
      }
    }
    finally {
...
<template>
  <div class="flex justify-content-between align-items-center">
    ...
    <IfCan :perms="['CHARGE-TYPE:CREATE']">
