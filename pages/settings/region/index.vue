...
// TABLE OPTIONS -----------------------------------------------------------------------------------------
const options = ref({
  tableName: 'Manage Region',
  moduleApi: 'settings',
  uriApi: 'manage-region',
  loading: false,
  actionsAsMenu: false,
  messageToDelete: 'Are you sure you want to delete the region: {{name}}?'
})
...
async function getItemById(id: string) {
  if (id) {
    ...
    try {
      ...
      if (error) {
        toast.add({ severity: 'error', summary: 'Error', detail: 'Region methods could not be loaded', life: 3000 })
      }
    }
    finally {
