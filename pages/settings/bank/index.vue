...
// TABLE OPTIONS -----------------------------------------------------------------------------------------
const options = ref({
  tableName: 'Manage Bank',
  moduleApi: 'settings',
  uriApi: 'manage-bank',
  loading: false,
  actionsAsMenu: false,
  messageToDelete: 'Are you sure you want to delete the bank: {{name}}?'
})
...
async function getItemById(id: string) {
  if (id) {
    ...
    try {
      ...
      if (error) {
        toast.add({ severity: 'error', summary: 'Error', detail: 'Bank methods could not be loaded', life: 3000 })
      }
    }
    finally {
