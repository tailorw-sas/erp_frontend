...
public class ConsumerUpdateManageHotelService {
    ...
    public void listen(UpdateManageHotelKafka objKafka) {
        try {
            UpdateManageHotelCommand command = new UpdateManageHotelCommand(objKafka.getId(), objKafka.getName(), objKafka.getIsApplyByVCC(), objKafka.getStatus(), objKafka.getAddress());
