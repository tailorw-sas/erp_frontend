...
public class ConsumerUpdateManagePaymentSourceService {
    ...
    public void listen(UpdateManagePaymentSourceKafka objKafka) {
        try {
            UpdateManagePaymentSourceCommand command = new UpdateManagePaymentSourceCommand(objKafka.getId(), objKafka.getName(), objKafka.getStatus(), objKafka.getExpense(), objKafka.getIsBank());
