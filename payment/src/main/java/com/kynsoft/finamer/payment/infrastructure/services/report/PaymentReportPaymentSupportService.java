...
public class PaymentReportPaymentSupportService implements IPaymentReport {
    ...
    public PaymentReportResponse generateReport(EPaymentReportType reportType, UUID paymentId) {
        ...
        try {
            ...
            document.open();
            document.addTitle("Report PAYMENT SUPPORT");
            document.addSubject("Using iText (usando iText)");
            document.addAuthor("FINAMER");
            Font chapterFont = new Font(Font.FontFamily.HELVETICA);
            Chunk chunk = new Chunk("PAYMENT SUPPORT REPORT", chapterFont);
            Chapter chapter = new Chapter(new Paragraph(chunk), 1);
            chapter.setNumberDepth(0);
            chapter.add(new Paragraph("PAYMENT SUPPORT REPORT", chapterFont));
        ...
        return new PaymentReportResponse("payment-suport",data);
    }
}
