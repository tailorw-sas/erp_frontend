...
public class ConsumerReplicateManagePaymentSourceService {
    ...
    public void listen(ReplicateManagePaymentSourceKafka objKafka) {
        try {
            CreateManagePaymentSourceCommand command = new CreateManagePaymentSourceCommand(objKafka.getId(), objKafka.getCode(), objKafka.getName(), objKafka.getStatus(), objKafka.getExpense(), objKafka.getIsBank());
