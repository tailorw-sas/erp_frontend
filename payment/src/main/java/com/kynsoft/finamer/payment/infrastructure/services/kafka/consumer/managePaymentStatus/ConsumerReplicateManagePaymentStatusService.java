...
public class ConsumerReplicateManagePaymentStatusService {
    ...
    public void listen(ReplicateManagePaymentStatusKafka objKafka) {
        try {
            CreateManagePaymentStatusCommand command = new CreateManagePaymentStatusCommand(objKafka.getId(), objKafka.getCode(), objKafka.getName(), objKafka.getStatus(), objKafka.getApplied(), objKafka.isConfirmed());
