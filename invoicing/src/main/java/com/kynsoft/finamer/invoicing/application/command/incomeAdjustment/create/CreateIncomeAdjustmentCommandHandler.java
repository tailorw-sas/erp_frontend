...
//        RulesChecker.checkRule(new ValidateObjectNotNullRule<>(command.getTransactionType(), "transactionType", "Manage Invoice Transaction Type ID cannot be null."));
        ...
        for (NewIncomeAdjustmentRequest adjustment : command.getAdjustments()) {
            ...
            RulesChecker.checkRule(new ManageInvoiceInvoiceDateInCloseOperationRule(this.closeOperationService, adjustment.getDate(), incomeDto.getHotel().getId()));
            ManageInvoiceTransactionTypeDto transactionTypeDto = adjustment.getTransactionType() != null ? this.transactionTypeService.findById(adjustment.getTransactionType()) : null;
