...
public interface ManageRoomRateWriteDataJPARepository extends JpaRepository<ManageRoomRate, UUID> {
    ...
    @Query(value = "SELECT * FROM fr_insert_room_rate(:p_id, " +
            ":p_adults, " +
            ":p_checkin, " +
            ":p_checkout, " +
            ":p_children, " +
            ...
            ":p_roomnumber, " +
            ":p_updatedat, " +
            ":p_booking)", nativeQuery = true)
    Map<String, Object> insertRoomRate(@Param("p_id") UUID id,
                                       ...
                                       @Param("p_rateadult") Double rateByAdult,
                                       @Param("p_ratechild") Double rateByChild,
